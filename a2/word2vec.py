...
def naiveSoftmaxLossAndGradient(
    ...
    softmax_values = softmax(np.dot(centerWordVec, outsideVectors.T))
    loss = -np.log(softmax_values[outsideWordIdx])
    gradCenterVec = sum(softmax_values[i] * outsideVectors[i] for i in range(len(outsideVectors))) - outsideVectors[outsideWordIdx]
    gradOutsideVecs = np.array([centerWordVec * yw for yw in softmax_values])
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (softmax_values[outsideWordIdx]-1)
...
def negSamplingLossAndGradient(
    ...
    A = -np.log(sigmoid(np.inner(uo, vc)))
    B = 0

    for i in negSampleWordIndices:
        B += np.log(sigmoid(-np.dot(outsideVectors[i], centerWordVec)))

    loss = A - B
    gradCenterVec = uo*(sigmoid(np.inner(uo, vc)) - 1)

    temp = 0
    for k in negSampleWordIndices:
        temp += U[k] * (1 - sigmoid(np.inner(-U[k], vc)))
    gradCenterVec += temp

    gradOutsideVecs = np.zeros_like(U)
    unique, counts = np.unique(negSampleWordIndices, return_counts=True)
    negSampleReps = dict(zip(unique, counts))

    for k in unique:
        gradOutsideVecs[k] = negSampleReps[k] * vc * (1 - sigmoid(np.inner(-U[k], vc)))
    
    gradOutsideVecs[outsideWordIdx] = vc * (sigmoid(np.inner(uo, vc)) - 1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    vci = word2Ind[currentCenterWord]
    vc = centerWordVectors[vci]
    for ow in outsideWords:
        owi = word2Ind[ow]
        l, gc, go = word2vecLossAndGradient(vc, owi, outsideVectors, dataset)
        loss = loss + l
        gradCenterVecs[vci] += gc
